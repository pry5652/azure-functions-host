...
namespace Microsoft.Azure.WebJobs.Script.WebHost
{
    public static class WebJobsApplicationBuilderExtension
    {
        ...
        public static IApplicationBuilder UseWebJobsScriptHost(this IApplicationBuilder builder, IApplicationLifetime applicationLifetime, Action<WebJobsRouteBuilder> routes)
        {
            ...
            // Specialization can change the CompatMode setting, so this must run later than specialization
            ...
            builder.UseWhen(context => HttpThrottleMiddleware.ShouldUse(context, httpOptions.CurrentValue), config =>
            {
            ...
            builder.UseWhen(VirtualFileSystemMiddleware.IsVirtualFileSystemRequest, config =>
            {
                config.UseMiddleware<VirtualFileSystemMiddleware>();
            });
